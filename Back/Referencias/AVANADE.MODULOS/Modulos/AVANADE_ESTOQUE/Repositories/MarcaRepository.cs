...
namespace AVANADE.MODULOS.Modulos.AVANADE_ESTOQUE.Repositories
{
    public class MarcaRepository<T> : BaseRepository<Marca>  where T: AvanadeDbContextComum<T>
    {
        ...
        public async Task<bool> NomeJaExiste(string nome, Guid? id = null) => id.HasValue ? await DbSet.AnyAsync(m => m.Nome == nome && m.Id != id.Value) : await DbSet.AnyAsync(m => m.Nome == nome);
    }
}
